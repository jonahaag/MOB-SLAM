function [mapPoints, directionAndDepth, vSetKeyFrames, newPointIdx, projectedPoints] = ProjectedHelperLocalBundleAdjustment(mapPoints, ...
    directionAndDepth, vSetKeyFrames, currKeyFrameId, intrinsics, newPointIdx, projectedPoints)
...
if ~isempty(outlierIdx)
    ...
    projectedPoints = removeProjectedPoints(projectedPoints, outlierIdx);
end

newPointIdx = newPointIdx - arrayfun(@(x) nnz(x>outlierIdx), newPointIdx);

%%
% % refinedPoses = zeros(vSetKeyFrames.NumViews,1);
% % reprojectionErrors = zeros(vSetKeyFrames.NumViews,1);
% for i = 1:vSetKeyFrames.NumViews
%     [pointIndices, featureIndices] = findWorldPointsInView(mapPoints, i);
%     xyzPoints = mapPoints.WorldPoints(pointIndices,:);
%     save indices.mat featureIndices pointIndices mapPoints vSetKeyFrames 
%     imagePoints = locations{i}(featureIndices,:);
%     camPose = vSetKeyFrames.Views.AbsolutePose(i);
%     [refinedPose, reprojectionErrors] = bundleAdjustmentMotion(xyzPoints, imagePoints, camPose, intrinsics,...
%         'PointsUndistorted', true, 'AbsoluteTolerance', 1e-7,...
%         'RelativeTolerance', 1e-16, 'MaxIteration', 100);
%     vSetKeyFrames = updateView(vSetKeyFrames, i, refinedPose);
% %     refinedPoses(i) = refinedPose;
% %     reprojectionErrors(i) = reprojectionError;
% 
% %%
%     maxError   = 6;
%     isInlier   = reprojectionErrors < maxError;
%     inlierIdx  = pointIndices(isInlier);
%     outlierIdx = pointIndices(~isInlier);
% 
%     % mapPoints = updateWorldPoints(mapPoints, inlierIdx, refinedPoints(isInlier,:));
%     % vSetKeyFrames = updateView(vSetKeyFrames, refinedPoses);
%     directionAndDepth = update(directionAndDepth, mapPoints, vSetKeyFrames.Views, inlierIdx, false);
