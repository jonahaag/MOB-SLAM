...
class QXboxController(QObject):
    ...
    def __init__(self):
        ...
        self.signaldictionary = {
            "SOUTH": lambda x: self.button_a_action.emit(x),
            ...
            "RY": lambda x: [
                self.__setattr__("_rjoyy", x / 32768),
                self.axis_rjoy_action.emit({"x": self._rjoyx, "y": self._rjoyy}),
            ],
            "RX": lambda x: [
                self.__setattr__("_rjoyx", x / 32768),
                self.axis_rjoy_action.emit({"x": self._rjoyx, "y": self._rjoyy}),
            ],
            "Y": lambda x: [
                self.__setattr__("_ljoyy", x / 32768),
                self.axis_ljoy_action.emit({"x": self._ljoyx, "y": self._ljoyy}),
            ],
            "X": lambda x: [
                self.__setattr__("_ljoyx", x / 32768),
                self.axis_ljoy_action.emit({"x": self._ljoyx, "y": self._ljoyy}),
            ],
    ...
    def start(self):
        while not self.connected and not self._stop:
            try:
                ...
                importlib.reload(
                    inputs
                )  # in case controller wasn't plugged in, this refreshes the module
                continue
            self.connected = True
